...
let array = [], alreadyPressed = [], winningArray = [], difficulty = 1, a = easy[0],
    b = easy[1], hearts = lives[0];
reloadGame();
...
function reloadGame() {
    switch (difficulty) {
        case 0:
            ...
            hearts = lives[0], array = [], alreadyPressed = [], winningArray = [];
            a = easy[0];
            b = easy[1];
            break;
        case 1:
            ...
            console.log("normal: ");
            console.log(mixedCards);
            hearts = lives[1], array = [], alreadyPressed = [], winningArray = [];
            a = normal[0];
            b = normal[1];
            break;
        case 2:
            ...
            console.log("hard: ");
            console.log(mixedCards);
            hearts = lives[2], array = [], alreadyPressed = [], winningArray = [];
            a = hard[0];
            b = hard[1];
            break;
        default:
...
if (!winningArray.includes(index)){
    if (alreadyPressed.includes(index)) {
        console.log("1");
        view.viewNumber(mixedCards[newClick[0]][newClick[1]][1], index);
        array.push(mixedCards[newClick[0]][newClick[1]][1]);
        ...
        if (array.length === 2) {
            ...
            if (array[0] === array[1]) {
                ...
                for (let l = 0; l < 2; l++){
                    console.log(alreadyPressed.length)
                    winningArray.push(alreadyPressed.pop());
                }
                if (winningArray.length === a * b){
                    view.createWinner();
                }
                array = [];
                ...
                console.log("4");
                console.log("lose");
                for (let i = 0; i < alreadyPressed.length; i++) {
                    view.hideNumber(alreadyPressed[i]);
                    console.log(alreadyPressed[i]);
                }
        ...
        console.log("5");
        view.viewNumber(mixedCards[newClick[0]][newClick[1]][0], index);
        if (alreadyPressed.length && !array.length) {
            console.log("6");
    ...
    if (alreadyPressed.length === 3 && array.length === 1) {
        console.log("7")
        console.log("hide: " + alreadyPressed[1]);
        view.hideNumber(alreadyPressed[1]);
        alreadyPressed.splice(1, 1);
    }
    console.log("array: " + array);
    console.log("alreadyPressed: " + alreadyPressed);
    console.log("win: " + winningArray)
}
